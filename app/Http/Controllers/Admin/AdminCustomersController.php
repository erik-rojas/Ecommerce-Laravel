...
{	
	...
	public function addcustomers(Request $request){
		...
		$customerData['industryTypes'] = $types;
	...
	public function addnewcustomers(Request $request){
		...
		if(count($existEmail)>0){
			...
			$customers_id = DB::table('customers')->insertGetId([
						...
						'customers_position'			=>	 $request->customers_position,
						...
						'customers_cellphone'			=>		$request->customers_cellphone,
	...
	public function editcustomers(Request $request){
		...
		$customerData['categories'] = $myVar->allCategories($language_id);
		...
		$customerData['subCategories'] = $subCategories;
		$customerData['mainCategories'] = $mainCategories;
	...
	public function updatecustomers(Request $request){
		if(session('customers_update')==0){
			...
			//delete categories
			DB::table('customers_to_categories')->where([
				'customers_id'  	=>   $customers_id,		 
				])->delete();
		
			foreach($request->categories as $categories){
				DB::table('customers_to_categories')->insert([
							'customers_id'   	=>     $customers_id,
